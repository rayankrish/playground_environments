...
class Poker(GameInterface):
    def __init__(self, game_id, players, game_type, parameters: PokerParameters, self_training=False):
    ...
    def after_hand_end(self):
        ...
        total_winnings = {}
        for pot_id, (amount, best_rank, winners_list) in settle_history.pot_winners.items():
    ...
    def get_state(self, player_sid = "", player_id = -1):
        ...
        for cur_player in self.players.values():
            ...
            for pot in self.game.pots:
                amounts_bet[cur_player.player_id] += pot.get_player_amount(cur_player.player_id)
